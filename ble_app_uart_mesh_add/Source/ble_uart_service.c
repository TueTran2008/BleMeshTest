...
#define DEVICE_NAME                     "TueIntegradeUartMesh"                               /**< Name of device. Will be included in the advertising data. */
...
static void node_nus_data_handler(ble_nus_evt_t *p_evt)
{
  ...
  else if(p_evt->type == BLE_NUS_EVT_COMM_STARTED)
  {
    ...
    if(m_is_node_request_data)
    {
      ...
      mesh_network_info_t send_to_node = xSystem.network_info;

      send_to_node.unicast_address.address_start = send_to_node.unicast_address.address_start + ACCESS_ELEMENT_COUNT;
      send_to_node.unicast_address.count = send_to_node.unicast_address.address_start + ACCESS_ELEMENT_COUNT;
      err_code = ble_nus_data_send(&m_nus, (uint8_t*)&send_to_node, &lenght, m_conn_handle);
