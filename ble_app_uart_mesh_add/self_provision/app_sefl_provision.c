...
static void subcribe_topic(access_model_handle_t model)
{
  ...
  err_code = dsm_address_publish_add(GROUP_ADDRESS_2, &m_group_addres_handle);
  APP_ERROR_CHECK(err_code);
...
// Add the address to the DSM as a subscription address:
  dsm_address_subscription_add(GROUP_ADDRESS_1, &m_group_addres_handle);
  dsm_address_subscription_add(GROUP_ADDRESS_2, &m_group_addres_handle);
  dsm_address_subscription_add(GROUP_ADDRESS_3, &m_group_addres_handle);
