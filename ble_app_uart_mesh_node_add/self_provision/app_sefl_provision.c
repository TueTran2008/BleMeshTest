...
static nrf_mesh_prov_provisioning_data_t provision_data = {
  ...
  .iv_index = 0,
  .address = LOCAL_ADDRESS_START + ACCESS_ELEMENT_COUNT,
...
void subcribe_topic(access_model_handle_t model)
{
  ...
  err_code = dsm_address_publish_add(GROUP_ADDRESS_1, &m_group_1_addres_handle);
  APP_ERROR_CHECK(err_code);
...
// Add the address to the DSM as a subscription address:
  ...
  dsm_address_subscription_add(GROUP_ADDRESS_2, &m_group_2_addres_handle);
  dsm_address_subscription_add(GROUP_ADDRESS_3, &m_group_3_addres_handle);
                        ...
                        dsm_local_unicast_address_t m_start_unicast_add)
{
  ...
  else
  {
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Found last provision Last unicast address %d\r\n", last_unicast.address_start);
